...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] List<ParticleSystem> TextVFX;
    [SerializeField] ParticleSystem EnhanceVFX;
    
    // [SerializeField] bool bTitle = false;
    
    int ClearChapterNumber = 0;
    int ClearStageNumber = 0; // 10개씩 끊어서 챕터로
    ...
    public int clearChapterNumber {get {return ClearChapterNumber;} set {ClearChapterNumber = value;}}
    ...
    public void StartLoadScene(int sceneindex) // 빌드 인덱스로
    {
    ...
    public void StartStage(int stageNum) // Stage 번호로로
    {
    ...
    {
        if(!EnemyInfos.ContainsKey(EnemyID)) return new EnemyData();

        return EnemyInfos[EnemyID];
    }

    public void PlayTextVFX(Vector3 pos)
    {
        int rand = Random.Range(0, TextVFX.Count);

        Instantiate(TextVFX[rand], pos, Quaternion.identity);
    }

    public void PlayEnhanceVFX(Vector3 pos)
    {
        Instantiate(EnhanceVFX, pos, Quaternion.identity);
    }

    public void OpenNextStage(int StageNum)
    {
        ClearStageNumber = Mathf.Max(ClearStageNumber, StageNum);
        clearChapterNumber =  ClearStageNumber / 10;
    }
}
