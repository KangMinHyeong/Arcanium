...
public class RollingSpawner : WeaponBase
{
    ...
    void SpwanRolling(int SpawnNum)
    {
        ...
        float Degree = (360.0f / SpawnNum)* Mathf.Deg2Rad;
        for(int i = 0; i<SpawnNum; i++)
        {
            float AddLocation_X = Mathf.Cos(Degree * i);
            float AddLocation_Y = Mathf.Sin(Degree * i);
            Vector3 Location = transform.position + new Vector3(AddLocation_X, 0.0f, AddLocation_Y) * (WeaponData.WeaponRange - 2.0f);
