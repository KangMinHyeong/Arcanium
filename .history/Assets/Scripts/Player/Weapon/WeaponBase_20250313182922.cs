...
public class WeaponBase : MonoBehaviour
{
    ...
    void InitWeaponData()
    {
        ...
        HitVFX = Resources.Load<ParticleSystem>(WeaponData.HitVFXPath);
    ...
    void UpdateSellCost()
    {
        ...
        switch (weaponState)
        {
            case WeaponState.Level_2:
                SellCost += WeaponData.EnhanceCostLevel_2 / 2;
                break;
            
            case WeaponState.Level_3:
                SellCost += WeaponData.EnhanceCostLevel_3 / 2;
                break;
        }
    ...
    {
        if(weaponState == WeaponState.Level_1)
        {
            EnhanceCostText.text = "   $    " + WeaponData.EnhanceCostLevel_2.ToString();
        }
        else if(weaponState == WeaponState.Level_2)
        {
            EnhanceCostText.text = "   $    " + WeaponData.EnhanceCostLevel_3.ToString();
        }
        else
        {
            EnhanceCostText.text = "Max Level";
            EnhanceCostText.fontSize = 135.0f;
            EnhanceCostText.alignment = TextAlignmentOptions.Midline;
        }        
    }

    protected virtual bool EnhanceWeapon()
    {
        // weaponState가 Level_3이면 강화완료 메세지 표시 후 얼리리턴
        ...
        // 현재 weaponState에 맞는 강화 코스트 업데이트트
        int requireCost = 0;
        switch (weaponState)
        {
    ...
    protected virtual void ReadyTrigger()
    {
    }
    ...
    protected virtual void PlayHitVFX(Vector3 Pos)
    {
